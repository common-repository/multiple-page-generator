...
class mpgCreatePages{

    function mpg_insertPages($POST){
    ...
    foreach ($pages as $page => $keys) {
        ...
        $meta = sanitize_text_field($POST['meta-description']);

        $seoTitle = sanitize_text_field($POST['seo-title']);

        $title = sanitize_text_field($POST['page-title']);

        $postStatus = sanitize_text_field($POST['postStatus']);
        ...
        update_post_meta($postId, '_yoast_wpseo_focuskw', $title);
